...
let blues = []
let greens = []
function pushIntegersToColourSet(colourSet, colourArray) {
    colourSet.map(set => {
        if (!isNaN(parseInt(set.charAt(set.length -1)))) {
            if(!isNaN(parseInt(set.charAt(set.length -2)))) {
                //2nd last digit is also a number
                if(colourArray === 'red') {
                    reds.push(set.substring(set.length -2, set.length))
                }
                if(colourArray === 'blue') {
                    blues.push(set.substring(set.length -2, set.length))
                }
                if(colourArray === 'green') {
                    greens.push(set.substring(set.length -2, set.length))
                }
            } else {
                if(colourArray === 'red') reds.push(set.substring(set.length -1, set.length))
                if(colourArray === 'blue') blues.push(set.substring(set.length -1, set.length))
                if(colourArray === 'green') greens.push(set.substring(set.length -1, set.length))
            }
        }
...
function filterOutImpossibleGames(gameInfo) {

    gameInfo.map(x => {
       const gameId = parseInt(x.split(":")[0].match(/\d+/)[0], 10)
        const game = x.split(":")[1].trim()
        console.log('og ', {game})

        let redSet = game.split(" red")
        pushIntegersToColourSet(redSet, 'red')
            //parseInt on each array value and if any of the ints are larger than 12, remove it
            reds.map(numberString => {
                numberString = parseInt(numberString)
            })
            if(reds.some(int => int > 12)) {
                //do nothing
            } else {
                let blueSet = game.split(" blue")
                pushIntegersToColourSet(blueSet, 'blue')

                blues.map(numberString => {
                numberString = parseInt(numberString)})

                if(blues.some(int => int > 14)) {
                    //do nothing
                } else {
                    let greenSet = game.split(" green")
                    pushIntegersToColourSet(greenSet, 'green')
                    greens.map(numberString => {
                        numberString = parseInt(numberString)
                    })
                    if(greens.some(int => int> 13)) {
                        //do nothing
                    } else {
                        if(numberOfValidGames.indexOf(gameId) === -1) numberOfValidGames.push(gameId)
                    }
                }
            }
            reds = []
